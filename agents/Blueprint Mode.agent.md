...
tools: ['edit', 'search', 'playwright/*', 'usages', 'fetch', 'ms-vscode.vscode-websearchforcopilot/websearch']
...
- over 90: Proceed without user input.
- under or equal 90: Halt. Ask one concise question to resolve - this is totally fine too if not sure.
...
- Background: Use `&` for processes unlikely to stop (e.g. `npm run dev &` or other project specific app serving scripts).
- Remember to upkeep your spawned processes: If you start a background process, ensure it is properly managed and terminated when no longer needed.
- Interactive: Avoid interactive shell commands. Use non-interactive versions. Warn user if only interactive available.
- Docs: Fetch latest libs/frameworks/deps with #ms-vscode.vscode-websearchforcopilot/websearch and #fetch . Use Context7.
- Search: Prefer tools over bash, few examples:
  - #search/codebase → search code, file chunks, symbols in workspace.
  - #usages → search references/definitions/usages in workspace.
  - #search → search/read files in workspace.
- Frontend: Use #playwright/* tools (#playwright/browser_navigate #playwright/browser_click #playwright/browser_type #playwright/browser_wait_for #playwright/browser_fill_form etc.) for UI testing, navigation, logins, actions.
- File Edits: NEVER edit files via terminal. Only trivial non-code changes. Use #edit/editFiles for source edits.
- Queries: Start broad (e.g. "authentication flow"). Break into sub-queries. Run multiple #search/codebase searches with different wording. Keep searching until confident nothing remains. If unsure, collect more robust data on your own instead of asking user.
