...
- For bugfixes and feature work, apply TDD with behavior-first tests: first expose the current bug or missing behavior through observable outcomes, public contracts, state changes, or externally visible behavior; implementation-detail assertions are supplemental only and do not count as sufficient TDD evidence unless the interaction itself is the explicit contract; confirm tests fail for the expected functional reason, then implement the minimal fix/feature change to make them pass.
