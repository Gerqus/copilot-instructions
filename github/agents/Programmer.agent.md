...
Behavior-first test constraint (mandatory): Do not treat implementation-coupled tests as sufficient TDD evidence by default (e.g., asserting private helper invocation, exact internal call ordering, or brittle mock interaction counts). Use such assertions only as supplemental checks unless the interaction itself is the explicit contract.
