...
metadata: {
  "skill": "verifier",
  "line": "codex",
  "counterpart": "/home/projekty/copilot-instructions/agents/Verifier.agent.md",
  "type": "agent-counterpart"
}
---

# Verifier (Codex Counterpart)

This skill is the Codex-native equivalent of `/home/projekty/copilot-instructions/agents/Verifier.agent.md`.
...
- Use `request_user_input (Plan mode) or direct user questions` to clarify acceptance criteria, confirm scope, discuss ambiguous results, and check in on priority between competing interpretations.
...
- **DoD baseline**: Check for `.agents/session/dod-<conversationId>.md` first. If it does not exist, check `.agents/session/dod.md`. If criteria are missing or provide only a test name/description, ask the user for explicit criteria **OR** infer structured DoD from the test file and linked docs (`docs/architecture.md`, etc.). Prioritize: user → `.agents/session/dod-<conversationId>.md` → `.agents/session/dod.md` → tests → docs.
...
- Acceptance criteria priority order: user request -> .agents/session/dod-<conversationId>.md -> .agents/session/dod.md -> tests -> docs.
